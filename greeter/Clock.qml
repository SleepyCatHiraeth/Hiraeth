...
// Large League Gothic clock with the date underneath, in the same voice as the
// AMBXST lockscreen so login and lock feel like one surface.
//
...
Column {
    ...
    readonly property string hours: Theme.use12h ? String(now.getHours() % 12 || 12).padStart(2, "0") : Qt.formatTime(now, "hh")
    readonly property string minutes: Qt.formatTime(now, "mm")

    component Glyph: RollingText {
        id: glyph
        property int order: 0
        readonly property real local: Theme.outCubic(Theme.span(root.rise, order * 0.09, order * 0.09 + 0.55))
    ...
    Row {
        ...
        Item {
            ...
            readonly property real local: Theme.outCubic(Theme.span(root.rise, 0.18, 0.7))
            opacity: local * pulse

            property real pulse: 1
            SequentialAnimation on pulse {
                running: Theme.base > 0
    ...
    Rectangle {
        anchors.horizontalCenter: parent.horizontalCenter
        width: 64 * root.unit * Theme.outQuint(Theme.span(root.rise, 0.35, 0.9))
        height: 3 * root.unit
        radius: height / 2
        color: Theme.primary
        opacity: 0.8
    ...
    Text {
        anchors.horizontalCenter: parent.horizontalCenter
        readonly property real local: Theme.outCubic(Theme.span(root.rise, 0.45, 1))
