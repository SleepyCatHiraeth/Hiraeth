...
Item {
    ...
    Rectangle {
        ...
        radius: LockStyle.pillRadius(height)
        color: Qt.rgba(0, 0, 0, 0.18)
        border.width: 1
        border.color: root.flash > 0
            ? Qt.rgba(LockStyle.error.r, LockStyle.error.g, LockStyle.error.b, 0.35 + 0.65 * root.flash)
            : Qt.rgba(LockStyle.overSurface.r, LockStyle.overSurface.g, LockStyle.overSurface.b, input.activeFocus ? 0.38 : 0.18)

        Behavior on border.color {
            enabled: LockStyle.base > 0 && root.flash === 0
            ColorAnimation { duration: LockStyle.dur(1) }
        }
        ...
        Text {
            ...
            color: root.flash > 0 ? LockStyle.error : LockStyle.primary
        ...
        Text {
            ...
            Behavior on opacity {
                ...
                NumberAnimation { duration: LockStyle.dur(0.6) }
            }
        }
        ...
        Item {
            ...
            Row {
                ...
                Behavior on x {
                    enabled: LockStyle.base > 0
                    NumberAnimation { duration: LockStyle.dur(0.6); easing.type: Easing.OutCubic }
                }
                ...
                Repeater {
                    ...
                    delegate: Text {
                        ...
                        font.family: LockStyle.mono
                        font.pixelSize: 15 * root.unit
                        color: settled ? LockStyle.overSurface : LockStyle.primary
                        ...
                        Behavior on color {
                            enabled: LockStyle.base > 0
                            ColorAnimation { duration: LockStyle.dur(0.8) }
                        }
                        Component.onCompleted: if (LockStyle.base === 0) { text = pick(pool); settled = true; }
                        Timer {
                            // Fast while arriving, then one change every ~0.5-0.8 s.
                            interval: glyph.settled ? 480 + Math.random() * 300 : 38
                            repeat: true
                            running: !glyph.settled || LockStyle.base > 0
                ...
                Rectangle {
                    ...
                    Timer {
                        ...
                        interval: 530
                        running: parent.visible && LockStyle.base > 0
                        repeat: true
                        onTriggered: on = !on
                    }
        ...
        Item {
            ...
            Behavior on shownAnim {
                enabled: LockStyle.base > 0
                NumberAnimation { duration: LockStyle.dur(1); easing.type: Easing.OutCubic }
            }
            ...
            Shape {
                ...
                Behavior on opacity { enabled: LockStyle.base > 0; NumberAnimation { duration: LockStyle.dur(0.6) } }
                Behavior on scale { enabled: LockStyle.base > 0; NumberAnimation { duration: LockStyle.dur(0.4) } }
                ShapePath {
                    strokeColor: LockStyle.overSurface
